...
object Huffman {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      ...
      def converter(x: List[(Char, Int)], acc: List[Leaf]): List[Leaf] = {
        if (x.isEmpty)
          acc
        else {
          val newacc = List[Leaf](Leaf(x.head._1, x.head._2)) ::: acc
          converter(x.tail, newacc)
        }
      }
      
      converter(x, List[Leaf]())
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      if (trees.length<2) trees
        ...
        List[CodeTree](newtree) ::: trees.tail.tail
      }
    }
   ...
   */
    ...
    // xxx = end condition (trees: List[CodeTree]): Boolean
    // yyy = combine function (trees: List[CodeTree]): List[CodeTree]
    // zzz = list
    
    def until(endCondition: List[CodeTree]=>Boolean, combineFunction: List[CodeTree]=>List[CodeTree])(list: List[CodeTree]): CodeTree = {
      if (singleton(list)) list.head
      else until(endCondition, combineFunction)(combineFunction(list))
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeacc(descendedTree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = {
      if(bits.isEmpty) {
        descendedTree match {
          case Leaf(char, weight) => acc ::: List[Char](char)
        }
      }
      else {
        descendedTree match {
          case Leaf(char, weight) => decodeacc(tree, bits, acc ::: List[Char](char))
    ...
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      def quickEncodeAcc(codeTable: CodeTable, text: List[Char], acc: List[Bit]): List[Bit] = {
        if (text.isEmpty) acc
          ...
          val newacc = acc ::: codeBits(codeTable)(text.head)
          quickEncodeAcc(codeTable, text.tail, newacc)
        }
      }
